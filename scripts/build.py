...
def sidebar(active):
    links = ''.join(f'<a href="{l["slug"]}.html" {"aria-current=page" if active == l["slug"] else ""}><span class="nav-number">{i:02}</span>{e(l["title"])}<span class="completion-dot" data-progress="{l["slug"]}"></span></a>' for i,l in enumerate(LESSONS,1))
    ...
    <p class="nav-label">01 / THE FUNDAMENTALS</p>{links}<p class="nav-label">02 / PUT IT INTO PRACTICE</p>
...
def lesson_rows():
    return ''.join(f'''<a class="lesson-row" href="{l['slug']}.html"><span class="row-number">{i:02}</span><div><h3>{e(l['title'])}</h3><p>{e(l['dek'])}</p></div><span class="lesson-time">{l['minutes']} min</span><span class="row-arrow" aria-hidden="true">↗</span></a>''' for i,l in enumerate(LESSONS,1))
...
<a class="flow-feature" href="predict-execution.html"><span class="eyebrow">YOUR NEXT PRACTICE</span><span><strong>Predict what Spark will do.</strong><small>Three snippets. Predict the tasks and data movement, reveal the reasoning, then verify it.</small></span><span aria-hidden="true">→</span></a>
...
for i,l in enumerate(LESSONS):
    ...
    practice_title = 'Change one assumption: turn AQE on' if l.get('exercises') else 'Try it on a small dataset'
